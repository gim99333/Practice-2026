...
// Переменные - рабочие элементы DOM
...
function updateCounterDisplay() {
    ...
    // Отображение сообщения о достижении экстремальных значений (в тех же цветовых тонах, что и окно счетчика)
    if (counter === 10 || counter === -10) {
        extremeMessage.style.visibility = 'visible';
        let infoColor = (counter === 10)? 'yellow' : 'green';
        extremeMessage.style.color = infoColor;
        extremeMessage.style.borderColor = infoColor;
    } else {
        extremeMessage.style.visibility = 'hidden';
    }
}
